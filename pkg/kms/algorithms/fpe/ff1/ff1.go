...
var (
	...
	ErrStringNotInRadix = errors.New("string is not within template/radix")
...
func (c Cipher) Encrypt(X []int) ([]int, error) {
...
func (c Cipher) EncryptWithTweak(X []int, tweak []byte) ([]int, error) {
	var ret []int
	var err error
	...
	if len(X) < minInputLen || len(X) > maxInputLen {
		return ret, fmt.Errorf("the length of X is not within the permitted range of %d .. %d : %d", minInputLen, maxInputLen, len(X))
	}
	if math.Pow(float64(c.radix), float64(len(X))) < 100 {
		return ret, fmt.Errorf("the length of X must be such that radix ^ length > 100 (radix ^ length = %f", math.Pow(float64(c.radix), float64(len(X))))
	}
	...
	*/
	...
	A, err = str(numA, radix, int(u))
	if err != nil {
		return ret, err
	}
	B, err = str(numB, radix, int(v))
	if err != nil {
		return ret, err
	}
	//A = numA.Text(radix)
...
func (c Cipher) Decrypt(X []int) ([]int, error) {
...
// override the tweak for each unique data input, which is a practical
// use-case of FPE for things like credit card numbers.
func (c Cipher) DecryptWithTweak(X []int, tweak []byte) ([]int, error) {
	var ret []int
	var err error
...
func (c Cipher) ciph(input []byte) ([]byte, error) {
	...
	// So, cache the potential error earlier
